<?php
    ...
    if(isset($_REQUEST["clanekSubmit"]))
    {
        ...
        if($pripona == "" || $soubor == "")
        {
            $_SESSION["error"] = "Nastala chyba s nahráváním souboru. Zkuste to prosím později.";
            header("Location: ../pridatClanekForm.php");
            exit();
        }
        // kontrola, zdali autor nahrál správný soubor obsahující článek
        if($pripona == ".doc" || $pripona == ".docx" || $pripona == ".odt" || $pripona == ".pdf")
        {
        ...
        // pokud nahraje špatný formát souboru
        else
        {
            // vrátím uživatele s chybou zpět
            $_SESSION["error"] = "Tento formát souboru s článkem není podporován!";
            header("Location: ../pridatClanekForm.php");
            exit();
        }
    }
